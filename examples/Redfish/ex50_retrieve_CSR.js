...
var FILE   = process.env.FILE;   // 'iloCSR.pem'
...
client.login(USER, PASS)
    ...
    .then((res) => {        
        var prevCSR = res.body.CertificateSigningRequest;
        if (res.body.CertificateSigningRequest) {
            var data = res.body.CertificateSigningRequest;
            console.log("CSR Data\n" + data);
            var bytes = fs.writeFileSync(FILE, data);    
        }
        else {
            console.log("...Generating... please wait then try again...");
        }
        return res
    })
