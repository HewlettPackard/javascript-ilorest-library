...
client.login(USER, PASS)
    ...
    .then((res) => {
        ...
        for (let item of res.body.Instances) {
            ...
            if(itemType && itemType.startsWith('#Power.')) {
                uri = itemUri;
            }
        }        
	...
	.then((res) => {
		console.log('EXAMPLE 30: Report PowerMetrics Average Watts');
		if ('PowerControl' in res.body && 'AverageConsumedWatts' in res.body['PowerControl'][0]['PowerMetrics']
			 && 'IntervalInMin' in res.body['PowerControl'][0]['PowerMetrics']) {
			console.log('AverageConsumedWatts = ' + res.body['PowerControl'][0]['PowerMetrics']['AverageConsumedWatts']
				 + ' watts over a ' + res.body['PowerControl'][0]['PowerMetrics']['IntervalInMin'] + ' minute moving average');
		} else {
			console.log('PowerMetrics resource does not contain \'AverageConsumedWatts\' or \'IntervalInMin\' property');
		}
	})
