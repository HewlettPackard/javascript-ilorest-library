...
client.login(USER, PASS)
	...
	.then((res) => {
		console.log('EXAMPLE 47: Clear AHS Data');
		var body = {'Action': 'ClearLog'};
		console.log('Post ' + JSON.stringify(body) + ' to ' + res.body['Oem']['Hp']['Links']['ActiveHealthSystem']['@odata.id']);
		return client.post(res.body['Oem']['Hp']['Links']['ActiveHealthSystem']['@odata.id'], body)
				.catch((err) => {
					console.log('Post ' + JSON.stringify(body) + ' to ' + res.body['Oem']['Hp']['Links']['ActiveHealthSystem']['@odata.id'] + ' Failed');
					throw err;
				});
	})
