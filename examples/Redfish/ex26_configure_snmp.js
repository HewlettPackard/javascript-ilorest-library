...
var snmp_mode = 'Agentless';
var snmp_alerts = false;
...
client.login(USER, PASS)
    ...
    .then((res) => {
		console.log('EXAMPLE 26: Configure iLO SNMP Settings');
        var uri;
        for (let item of res.body.Instances) {
            if(item['@odata.type'] && item['@odata.type'].startsWith('#SnmpService.')) {
                uri = item['@odata.id'];
				var body = {'Mode': snmp_mode, 'AlertsEnabled': snmp_alerts};
				console.log('Patch ' + JSON.stringify(body) + ' to ' + item['@odata.id']);
				return client.patch(item['@odata.id'], body).catch((err) => {
					console.log('Patch ' + JSON.stringify(body) + ' to ' + itemUri + ' Failed');
					throw err;
				});
            }
        }        
