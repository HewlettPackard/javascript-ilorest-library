...
client.login(USER, PASS)
    ...
    .then((res) => {
        ...
        for (let item of res.body.Instances) {
            ...
            if(itemType && itemType.startsWith('#LogService.') && itemUri.endsWith('IML/')) {
                uri = itemUri;
            }
        }        
	...
	.spread(function () {
		console.log('EXAMPLE 22: Dump Integrated Management Log');
		for (let res of arguments) {
			console.log(res.body['Severity'] + ': Class ' + res.body['Oem']['Hp']['Class'] + ' / Code ' + res.body['Oem']['Hp']['Code'] + ':\t' + res.body['Message']);
