...
client.login(USER, PASS)
	...
	.then((res) => {
		console.log('EXAMPLE 19: Reset iLO');
		var body = {'Action': 'Reset'};
		console.log('Post ' + JSON.stringify(body) + ' to ' + res.body['@odata.id']);
		return client.post(res.body['@odata.id'], body).catch((err) => {
			console.log('Post to ' + res.body['@odata.id'] + ' Failed');
			throw err;
		});
	})
