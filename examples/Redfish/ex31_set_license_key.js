...
client.login(USER, PASS)
	...
	.then((res) => {
		console.log('EXAMPLE 31: Set iLO License Key');
		var body = {};
		body['LicenseKey'] = iLO_Key;
		console.log('Post ' + JSON.stringify(body) + ' to ' + res.body['Oem']['Hp']['Links']['LicenseService']['@odata.id']);
		return client.post(res.body['Oem']['Hp']['Links']['LicenseService']['@odata.id'], body)
				.catch((err) => {
					console.log('Post ' + JSON.stringify(body) + ' to ' + res.body['Oem']['Hp']['Links']['LicenseService']['@odata.id'] + ' Failed');
					throw err;
				});
	})
