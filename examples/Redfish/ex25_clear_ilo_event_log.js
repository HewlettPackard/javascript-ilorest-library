...
client.login(USER, PASS)
    ...
    .then((res) => {
		console.log('EXAMPLE 25: Clear iLO Event Log');
		var uri;
        for (let item of res.body.Instances) {
            if(item['@odata.type'] && item['@odata.type'].startsWith('#LogService.') && item['@odata.id'].endsWith('IEL/')) {
                uri = item['@odata.id'];
				var body = {'Action': 'ClearLog'};
				console.log('Post ' + JSON.stringify(body) + ' to ' + item['@odata.id']);
				return client.post(item['@odata.id'], body).catch((err) => {
					console.log('Post ' + JSON.stringify(body) + ' to ' + item['@odata.id'] + ' Failed');
					throw err;
				});
            }
        }        
