...
client.login(USER, PASS)
	...
	.then((res) => {
		if ('#HpESKM.ClearESKMLog' in res.body['Actions']) {
			var post_target = res.body['Actions']['#HpESKM.ClearESKMLog']['target'];
		}

		console.log('EXAMPLE 40: Reset ESKM event logs');
		var body = {};
		body['Action'] = 'ClearESKMLog';

		console.log('Patch ' + JSON.stringify(body) + ' to ' + post_target);
		return client.patch(post_target, body)
				.catch((err) => {
					console.log('Patch ' + JSON.stringify(body) + ' to ' + post_target + ' Failed');
					throw err;
				});
	})
