...
client.login(USER, PASS)
    ...
    .then((res) => {
        ...
        for (let item of res.body.Instances) {
            ...
            if(itemType && itemType.startsWith('#HpeiLOLicense.')) {
                uri = itemUri;
            }
        }        
        ...
        return client.get(uri).catch((err) => {
            console.log('\n  Get Hp iLO License Failed\n');
            throw err;
        });
    })

    .then((res) => {
            console.log('EXAMPLE 45: Get iLO License Key');
            var licenseproperties = ['License', 'LicenseKey', 'LicenseType'];
            for (let licenseproperty of licenseproperties) {
                    console.log(licenseproperty + ': ' + res.body[licenseproperty]);
            }
    })
