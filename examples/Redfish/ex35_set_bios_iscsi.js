...
var bios_properties = {'iSCSIBootAttemptInstance': 2,
...
client.login(USER, PASS)
    ...
    .then((res) => {
        ...
        for (let item of res.body.Instances) {
            ...
            if(itemType && itemType.startsWith('#HpiSCSISoftwareInitiator.')) {
                uri = itemUri;
            }
        }        
        ...
        return client.get(uri).catch((err) => {
            console.log('\n  Get Hp ISCSI Software Initiator Failed\n');
            throw err;
        });
    })

	.then((res) => {
		console.log('EXAMPLE 35: Set ISCSI Config');
		console.log('Patch ' + JSON.stringify(bios_properties) + ' to ' + res.body['href']);
		return client.patch(res.body['href'], bios_properties, bios_password)
				.catch((err) => {
					console.log('Patch ' + JSON.stringify(bios_properties) + ' to ' + res.body['href'] + ' Failed');
					throw err;
				});
	})
